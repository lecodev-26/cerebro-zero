...
class EntrenadorProfundo:
    ...
    def entrenar_epoch(self, X, y):
        ...
        error = y - salida
        grad = error * salida * (1 - salida)  # Sigmoid derivada
        
        # Backpropagation
        for i in range(len(self.cerebro.pesos)-1, -1, -1):
            if i > 0:
                # Derivada ReLU para capas ocultas
...
# DATOS COMPLEJOS: XOR con más ejemplos
print("🧠 CREANDO CEREBRO PROFUNDO")
print("="*50)

# 2 entradas, 64 ocultas, 32 ocultas, 16 ocultas, 1 salida (5 capas)
cerebro = CerebroProfundo([2, 64, 32, 16, 1])
cerebro.resumen()

# Datos XOR con más variación
X = np.array([[0,0], [0,1], [1,0], [1,1]])
y = np.array([[0], [1], [1], [0]])

# Repetir datos para mejor entrenamiento
